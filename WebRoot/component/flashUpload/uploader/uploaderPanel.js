...
keel.UploadPanel = function(cfg) {
	...
	this.addEvents({// 加入自定义事件
		"beforeUploadEvent" : true,
		'uploadcompleteEvent' : true,
		"beforeAddFiles":true
	});
...
Ext.extend(keel.UploadPanel, Ext.Panel, {
	...
	onUploadComplete : function(file) {
		...
		if (file.filestatus == -4||file.filestatus == -3) {
		//if (file.filestatus == -4) {
			...
			for (var i = 0; i < ds.getCount(); i++) {
				...
				if (record.get('id') == file.id) {
					...
					me.fireEvent('uploadcompleteEvent', ds, file.name, i,
							record.get('state'));
					record.commit();
				}
			}
	...
	addFileToQueue : function(file) {
		var me = this;
		var store = me.gp.getStore();
		for (var i = 0; i < store.getCount(); i++) {
			if (store.getAt(i).get('name') == file.name) {
				Ext.Msg.show({
							title : '重名',
							msg : '不能添加重名文档',
							buttons : Ext.Msg.OK,
		...
		var flag=me.fireEvent('beforeAddFiles', file);
		if(!flag){
				me.swfupload.cancelUpload(file.id, false);
				me.swfupload.uploadStopped = false;
				return;
	...
	onFileQueued : function(file) {
		var me = this.customSettings.scope_handler;
		me.fireEvent('beforeUploadEvent', file);
		me.addFileToQueue(file);
	},
	onUploadSuccess : function(file, serverData) {
		var me = this.customSettings.scope_handler;
		var ds = me.gp.store;
		console.info("serverData : ",serverData);
