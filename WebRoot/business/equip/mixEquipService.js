Ext.namespace('dm.mixEquip');

dm.mixEquip.GridPanel = function() {
	var gridPanel = this;
	var grid = gridPanel;
	...
	gridPanel.store = new Ext.data.Store({
				proxy : new Ext.data.HttpProxy({
							url : 'resource/MixEquipService/search',
							method : 'post'
						}),
	...
	dm.mixEquip.GridPanel.superclass.constructor.call(this, {
				autoWidth : true,
				region : 'center',
				loadMask : {
					msg : '正在加载...'
					 ...
					 getRowClass : function(record,rowIndex,rowParams,store){
		                   ...
		                   if(record.data.pileStatus){
		                	   switch(record.data.pileStatus){
		                	   	case '在线':return 'x-grid-record-green';
		                	   }
	...
	gridPanel.on('rowdblclick',function(a,rowIndex,e) {
	    var row = gridPanel.getSelectionModel().getSelected().data;
	    if(row.equipmentStatus=='离线'){
	    	alert("设备已离线！")
	    }else{
	    	var win = new dm.mixEquip.win(row.equipmentCode);
	    	win.show();
	    }
...
Ext.extend(dm.mixEquip.GridPanel, Ext.grid.GridPanel, {
	loadData : function() {
		var gridPanel = this;
		gridPanel.store.load({
					params : {
	...
	addWin : function() {
		var gird = this;
		var win = new dm.mixEquip.addWin("add", null,gird);
		win.show();
	},
	updWin : function() {
		var gird = this;
		...
		var win = new dm.mixEquip.addWin("upd", id,gird);
		win.show();
	},
	infoWin : function() {
		var userGird = this;
		...
		var id = this.getSelectionModel().getSelected().data.equipmentId;
		var win = new dm.mixEquip.addWin("info", id);
		win.show();
	},
	del : function() {
		var gird = this;
		...
		var id = this.getSelectionModel().getSelected().data.equipmentId;
		Ext.Ajax.request({
			url : 'resource/MixEquipService/del',
			method : 'POST',
			params : {
				'id':id
			},
...
dm.mixEquip.win = function(pileDriverNumber){
	var win = this;
	dm.mixEquip.win.superclass.constructor.call(this, {
		fitToFrame: true,  
		width:1300,
		height:530,
        html: '<iframe  src="business/equip/mixEquip.html?pileDriverNumber='+pileDriverNumber+'" frameborder="0" width="100%" height="100%"></iframe>',
		closable : true
	});
};
Ext.extend(dm.mixEquip.win, Ext.Window);

dm.mixEquip.addWin = function(actionType, id,gridPanel) {
	var win = this;
					...
					if (actionType == "add") {
						...
						if (win.form.getForm().isValid()) {
							...
							win.form.getForm().doAction('submit', {
								url : "resource/MixEquipService/add",
								method : 'post',
								waitMsg : '正在加载',
								timeout : 2000,
						...
						if (win.form.getForm().isValid()) {
							win.form.getForm().doAction('submit', {
								url : "resource/MixEquipService/modify",
								method : 'post',
								params : {
									id:id
								},
	...
	if (actionType == "add") {
		...
		win.form.getForm().load({
			...
			url : "resource/MixEquipService/load"
		});
	} else if (actionType == "info") {
		actionTitle = '查看';
		win.form.getForm().load({
	...
	dm.mixEquip.addWin.superclass.constructor.call(this, {
		title : actionTitle,
		resizable : false,
		modal : true,
		width:385,
...
Ext.extend(dm.mixEquip.addWin, Ext.Window);