Ext.namespace('dm.recordChange');

dm.recordChange.grid = function(){
	var grid = this;
	...
	grid.store = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
			url : 'resource/recordChange/search',
			method : 'post'
		}),
	...
	dm.recordChange.grid.superclass.constructor.call(this, {
		title : '文件发布',
		closable:true,
		viewConfig : {
			forceFit : true
...
Ext.extend(dm.recordChange.grid,Ext.grid.GridPanel,{
	loadData : function() {
		var grid = this;
		grid.getStore().load({
			params : {
	...
	addWin : function(){
		...
		var win = new dm.recordChange.addWin("add", null,gird);
		win.show();
	},
	updWin : function(){
		var gird = this;
		...
		var win = new dm.recordChange.addWin("upd", id,gird);
		win.show();
	},
	infoWin : function(){
		var gird = this;
		...
		var id = this.getSelectionModel().getSelected().data.id;
		var win = new dm.recordChange.addWin("info", id,gird);
		win.show();
	},
	del : function(){
		var gird = this;
		...
		var id = this.getSelectionModel().getSelected().data.id;
		Ext.Ajax.request({
			url : 'resource/recordChange/delete',
			method : 'POST',
			params : {
				id:id
			},
...
dm.recordChange.addWin = function(actionType, id,grid){
	var win = this;
			...
			listeners : {
				'click' : function(){
					if (actionType == "add") {
						...
						if (win.form.getForm().isValid()) {
							win.form.getForm().doAction('submit', {
								url : "resource/recordChange/modify",
								method : 'post',
								params:{id:id},
								waitMsg : '正在加载',
								timeout : 2000,
	...
	if (actionType == "add") {
		...
		win.form.getForm().load({
			...
			url : "resource/recordChange/load"
		});
	...
	dm.recordChange.addWin.superclass.constructor.call(this, {
		title : '新增',
		resizable : false,
		bodyStyle : 'padding:5px 0px 0px',
		modal : true,
...
Ext.extend(dm.recordChange.addWin, Ext.Window);
