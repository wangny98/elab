Ext.namespace('dm.recordRelease');

dm.recordRelease.grid = function(){
	var grid = this;
	...
	grid.store = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
			url : 'resource/RecordRelease/search',
			method : 'post'
		}),
	...
	dm.recordRelease.grid.superclass.constructor.call(this, {
		title : '文件发布',
		closable:true,
		viewConfig : {
			forceFit : true
...
Ext.extend(dm.recordRelease.grid,Ext.grid.GridPanel,{
	loadData : function() {
		var grid = this;
		grid.getStore().load({
			params : {
	...
	addWin : function(){
		...
		var win = new dm.recordRelease.addWin("add", null,gird);
		win.show();
	},
	updWin : function(){
		var gird = this;
		...
		var win = new dm.recordRelease.addWin("upd", id,gird);
		win.show();
	},
	infoWin : function(){
		var gird = this;
		...
		var id = this.getSelectionModel().getSelected().data.id;
		var win = new dm.recordRelease.addWin("info", id,gird);
		win.show();
	},
	del : function(){
		var gird = this;
		...
		var id = this.getSelectionModel().getSelected().data.id;
		Ext.Ajax.request({
			url : 'resource/RecordRelease/delete',
			method : 'POST',
			params : {
				id:id
			},
...
dm.recordRelease.addWin = function(actionType, id,grid){
	var win = this;
					...
					if (actionType == "add") {
						...
						if (win.form.getForm().isValid()) {
							win.form.getForm().doAction('submit', {
								url : "resource/RecordRelease/add",
								method : 'post',
								waitMsg : '正在加载',
								timeout : 2000,
						...
						if (win.form.getForm().isValid()) {
							win.form.getForm().doAction('submit', {
								url : "resource/RecordRelease/modify",
								method : 'post',
								params:{id:id},
								waitMsg : '正在加载',
								timeout : 2000,
	...
	if (actionType == "add") {
		...
		win.form.getForm().load({
			...
			url : "resource/RecordRelease/load"
		});
	...
	dm.recordRelease.addWin.superclass.constructor.call(this, {
		title : '新增',
		resizable : false,
		bodyStyle : 'padding:5px 0px 0px',
		modal : true,
...
Ext.extend(dm.recordRelease.addWin, Ext.Window);
