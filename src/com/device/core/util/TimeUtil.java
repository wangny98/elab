...
public final class TimeUtil implements Serializable {
    ...
    public TimeUtil(String time, String timeSeparator) {
        ...
        String matcher = new StringBuffer().append("\\d+?").append(pattern).append("\\d+?").append(pattern).append(
                "\\d+?").toString();
    ...
    public String toString() {
        ...
        return new StringBuffer().append(fields[DAY]).append(", ").append(df.format(fields[HOUR]))
                .append(timeSeparator).append(df.format(fields[MINUTE])).append(timeSeparator).append(
     ...
     * @param second  
     *            秒  
     * @return String 包含小时、分钟、秒的时间字符串，例如3小时23分钟13秒。  
     */
